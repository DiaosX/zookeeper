...
{
    public class GetDataRequest : IRecord, System.IComparable
	{
		...
		public int CompareTo(object obj)
		{
			...
			int num = this.Path.CompareTo(getDataRequest.Path);
			if (num != 0)
			{
				return num;
			}
			num = ((this.Watch == getDataRequest.Watch) ? 0 : (this.Watch ? 1 : -1));
			if (num != 0)
			{
				return num;
			}
		...
		public override bool Equals(object obj)
		{
			...
			if (object.ReferenceEquals(getDataRequest, this))
			{
				return true;
			}
			bool flag = this.Path.Equals(getDataRequest.Path);
			if (!flag)
			{
				return flag;
			}
			flag = (this.Watch == getDataRequest.Watch);
			if (!flag)
			{
				return flag;
			}
