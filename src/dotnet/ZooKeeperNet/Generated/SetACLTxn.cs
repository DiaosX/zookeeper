...
{
    public class SetACLTxn : IRecord, System.IComparable
	{
		...
		public SetACLTxn(string path, System.Collections.Generic.IEnumerable<ACL> acl, int version)
		{
			this.Path = path;
			this.Acl = acl;
		...
		{
			throw new System.InvalidOperationException("comparing SetACLTxn is unimplemented");
		}
		...
		public override bool Equals(object obj)
		{
			...
			if (object.ReferenceEquals(setACLTxn, this))
			{
				return true;
			}
			bool flag = this.Path.Equals(setACLTxn.Path);
			if (!flag)
			{
				return flag;
			}
			flag = this.Acl.Equals(setACLTxn.Acl);
			if (!flag)
			{
				return flag;
			}
			flag = (this.Version == setACLTxn.Version);
			if (!flag)
			{
				return flag;
			}
